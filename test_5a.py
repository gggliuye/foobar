...
# of a quasar quantum flux field. In order to make the jump to hyperspace, you need to know the configuration of
# celestial bodies in the quadrant you plan to jump through. In order to do *that*, you need to figure out how many
...
# of the rows and columns do not matter (and can thus be freely swapped around). Star grid standardization means that
# the width and height of the grid will always be between 1 and 12, inclusive. And while there are a variety of
# celestial bodies in each grid, the number of states of those bodies is between 2 and 20, inclusive. The solution
# can be over 20 digits long, so return it as a decimal string.  The intermediate values can also be large, so you
...
# 00
#
# In the above configuration, all celestial bodies are "silent" - that is, they have a state of 0 - so any swap of row
# or column would keep it in the same state.
#
# 00 00 01 10
# 01 10 00 00
#
...
# moves them between the top and bottom.  In both, the *groupings* are the same: one row with two bodies in state 0,
